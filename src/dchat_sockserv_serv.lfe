...
(defun start_link (listen-socket accept-callback msg-callback)
  (gen_server:start_link (MODULE)
                         (tuple listen-socket accept-callback msg-callback)
                         ()))
...
(defun init
  (((tuple listen-socket accept-callback msg-callback))
   ...
   (tuple 'ok (make-state msg-callback msg-callback))))
  ...
  (case (gen_tcp:accept listen-socket)
    ((tuple 'ok socket)
     (error_logger:info_msg "~p (~p) accepted new connection~n"
                            (list (MODULE) (self)))
     (apply accept-callback (list (self)))
...
(defun handle_msg (msg-callback msg state)
  ...
  (apply msg-callback (list (self) msg))
  (tuple 'noreply state))
